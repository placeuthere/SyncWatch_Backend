/**
 * 监听页面 <video> 的 play、pause、seek，上报 user-action；
 * 接收 background 的 SYNC_VIDEO 并应用，用 isRemoteAction 避免回传。
 */
console.log('%c [一起看] 间谍脚本已成功加载！', 'color: white; background: green; padding: 5px;');

const remoteActionFlags = new WeakMap();
let hasShownInvalidatedTip = false;

function showInvalidatedTip() {
  if (hasShownInvalidatedTip) return;
  hasShownInvalidatedTip = true;
  alert('插件已更新，请刷新页面');
}
...
function setupVideoListener(video) {
  ...
  video.addEventListener('play', (e) => {
    console.log('[一起看] 捕获到视频动作:', e.type);
    sendUserAction('play');
  });
...
chrome.runtime.onMessage.addListener((message) => {
  ...
  if (message?.type === 'SYNC_VIDEO' && message.payload != null) {
    const videos = document.querySelectorAll('video');
    if (videos.length) applySyncToVideo(videos[0], message.payload);
    return;
  }
